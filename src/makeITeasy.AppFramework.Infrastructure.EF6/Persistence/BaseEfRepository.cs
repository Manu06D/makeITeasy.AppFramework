...
namespace makeITeasy.AppFramework.Infrastructure.Persistence
{
    public abstract class BaseEfRepository<T, U> : IAsyncRepository<T> where T : class, IBaseEntity where U : DbContext
    {
        ...
        public virtual async Task<QueryResult<X>> ListWithProjectionAsync<X>(ISpecification<T> spec, bool includeCount = false, CancellationToken cancellationToken = default) 
            where X : class
        {
